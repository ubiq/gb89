    ...
    if (provider) {
        ...
        web3js.eth.getChainId().then((id) => {
            if (id !== validNetworkId) {
                $(".status-message").html(`<i class="fa fa-exclamation-circle"></i> Not connected to Ubiq main network.`);
                return;
            }
        });
        contract = new web3js.eth.Contract(contractABI, contractAddress);
    } else {
        console.warn("Please install the Sparrow or MetaMask extension and connect your wallet.");
    }
...
function handleAccountsChanged(accounts) {
    if (accounts.length === 0) {
        console.log("Sparrow/MetaMask is locked or you haven't connected any account. Please connect to MetaMask.");
        $(".mint button").hide();
    ...
    ethereum
        ...
        .catch((err) => {
            if (err.code === 4001) {
                ...
                console.warn('Connect to Sparrow/MetaMask.');
            } else {
                console.error(err);
            }
        });
...
$(document).ready(function () {
    if (window.ethereum == null) {
        $(".status-message").html(`<i class="fa fa-exclamation-circle"></i> Sparrow/MetaMask isn't installed`);
        $(".mint button").hide();
    } else {
